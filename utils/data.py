...
class Data:
        ...
        for idx in range(len(in_lines)):
            ...
            if len(line) > 2:
                ...
                if idx < len(in_lines) - 1 and len(in_lines[idx+1]) > 2:
                    mid_info = in_lines[idx+1].strip().split()[0]
                    try:
                        biword = word + mid_info.decode('utf-8')
                    except (UnicodeDecodeError, AttributeError):
                        biword = word + mid_info
                else:
                    biword = word + NULLKEY
    ...
    def build_gaz_file(self, gaz_file):
        ...
        if gaz_file:
            ...
            for fin in fins:
                fin = fin.strip().split()[0].encode('utf-8')
                if fin:
    ...
    def build_gaz_alphabet(self, input_file):
        ...
        for line in in_lines:
            if len(line) > 3:
                word = line.split()[0].encode('utf-8')
                if self.number_normalized:
                    word = normalize_word(word)
